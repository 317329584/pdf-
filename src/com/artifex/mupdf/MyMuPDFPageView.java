...
public class MyMuPDFPageView extends ViewGroup {
	...
	public int DefalueHeight = 1740;

	public MyMuPDFPageView(BaseActivity context,MuPDFCore pdfCore,Point parentSize) {
		super(context);
	...
	public void drawOncePage(int pdfPageNum, PointF pdfPageSize) {
		...
		float sourceScale = Math.min(parentSize.x/pdfPageSize.x, parentSize.y/pdfPageSize.y);//计算缩放比例
		Point newSize = new Point((int)(pdfPageSize.x * sourceScale), (int)(pdfPageSize.y * sourceScale));
		mSize = newSize;
		if (null == mEntireBm || (null != mEntireBm && mEntireBm.getWidth() != newSize.x) || (null != mEntireBm && mEntireBm.getHeight() != newSize.y)) {
	...
	protected void drawPage(Bitmap bm, int sizeX, int sizeY,int patchX, int patchY, int patchWidth, int patchHeight) {//异步栈调NDK方法画内容
		pdfCore.drawPage(curPageNum, bm, sizeX, sizeY, patchX, patchY, patchWidth, patchHeight);
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (null != busyLoadingIndicator) {
			int bw = 100,bh = 100;
			busyLoadingIndicator.layout((w-bw)/2, (h-bh)/2, (w+bw)/2, (h+bh)/2);
		}
	}
			...
			mDrawPatch = new AsyncTask<PatchInfo,Void,PatchInfo>() {
				protected PatchInfo doInBackground(PatchInfo... v) {
					drawPage(v[0].bm, v[0].patchViewSize.x, v[0].patchViewSize.y,
									  v[0].patchArea.left, v[0].patchArea.top,v[0].patchArea.width(), v[0].patchArea.height());
					return v[0];
				}
